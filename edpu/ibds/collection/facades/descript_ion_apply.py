...
def _descript_ion_apply(data_dir: str, collection_name: str, data_path: str, use_descript_ion: bool) -> None:
    if not use_descript_ion:
        return

    from ...impl.descript_ion_apply import descript_ion_apply as impl
...
def descript_ion_apply(data_dir: str, collection_dict: CollectionDict, storage_device_: StorageDevice) -> None:
    ...
    for collection_name, data in collection_dict_items:
        ...
        for location in data[COLLECTION_VALUE_LOCATIONS]:
            if location.getStorageDevice().getName() == storage_device_.getName():
                ...
                _descript_ion_apply(data_dir, collection_name, path_prefix + location.getPath(), data[COLLECTION_VALUE_USE_DESCRIPT_ION])
